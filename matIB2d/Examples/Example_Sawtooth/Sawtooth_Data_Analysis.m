...
function Sawtooth_Data_Analysis()
...
for i=start:1:finish
        ...
        xBuff = 5; % how wide of region to average over ("xBuff-boxes wide")
        ...
        inds_UV = give_Inds_For_U_V(inds,xBuff,yBuff);
        
        % get flows left/right
        [flow_L, flow_R] = get_Average_Flows_Left_Right(inds_UV,U);
        ...
        vol_MAT(i,1) = flow_L * vert_dist / G;
...
function [flow_L, flow_R] = get_Average_Flows_Left_Right(inds_UV,U)

len = length(inds_UV(:,1));

mean_L = 0;
mean_R = 0;
for i=1:len
    mean_L = mean_L + U( inds_UV(i,1), inds_UV(i,2) );
    mean_R = mean_R + U( inds_UV(i,1), inds_UV(i,3) );
end
...
function inds_UV = give_Inds_For_U_V(inds,xBuff,yBuff)
    ...
    inds_horR = [];
    for i=1:xBuff
        inds_hor = [inds_hor; (inds(1)-2+(i-1) )*ones(lenV,1)];
        inds_horR = [inds_horR; (inds(2)+2-(i-1) )*ones(lenV,1)];
    end
   
    inds_Vert_OG = inds_Vert;
    for i=2:xBuff
       inds_Vert = [inds_Vert; inds_Vert_OG]; 
    end
