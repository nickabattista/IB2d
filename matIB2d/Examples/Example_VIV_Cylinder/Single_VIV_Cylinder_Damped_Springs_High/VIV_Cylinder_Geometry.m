...
x0 = 0.5;                      % x-Center for Cylinder
y0 = 0.5;                      % y-Center for Cylinder
r = w/6;                       % Radii of Cylinder
struct_name = 'viv';       % Name for .vertex, .spring, etc files.
...
%
% Translate Geometry Downwards
%
xLag_Cy = xLag_Cy + Lx/7;
yLag_Cy = yLag_Cy - 0.375;
yLag = yLag - 0.375;

%
% After Translating Downwards
%
plot(xLag(1:end/2),yLag(1:end/2),'k-'); hold on;
plot(xLag(end/2+1:end),yLag(end/2+1:end),'k-'); hold on;
plot(xLag_Cy,yLag_Cy,'r*'); hold on;
plot(xLag_Cy(1),yLag_Cy(1),'g*'); hold on;
plot(xLag(indsTether(1)),yLag(indsTether(1)),'b*'); hold on;
plot(xLag(indsTether(2)),yLag(indsTether(2)),'g*'); hold on;
...
% Prints .spring file!
k_Spring = 1.5e7; resting_length_tether = 2*r;
offset = length(xLag);
...
function print_Lagrangian_Damped_Springs(xLag,yLag,k_Spring,ds_Rest,r,offset,indsTether,resting_length_tether,b_damp,struct_name)

    N = length(xLag); % just two damped springs to connect cylinder to channel

    dSpring_fid = fopen([struct_name '.d_spring'], 'w');
