...
period = 0.025;                      % Time it takes to move to the right
L = 0.6;                             % Distance cylinder moves in each direction
...
if current_time <= period
    ...
    xPts(IDs) = xPts(IDs) + tanh(200*current_time)*s*dt;
    targets(IDs,2) = xPts(IDs);
