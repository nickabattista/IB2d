...
% CONSTANT VELOCITY
%uX_Tar = -Umax * (2*tanh(t)) * ( (Ly/2+w/2) - y )*( (Ly/2-w/2) - y ); % Only external forces in x-direction

% PULSATILE VELOCITY PROFILE
freq = 2;
uX_Tar = -Umax * (sin(freq*2*pi*t)) * ( (Ly/2+w/2) - y )*( (Ly/2-w/2) - y ); % Only external forces in x-direction

% PULSATILE VELOCITY PROFILE w/ PAUSES BETWEEN SUCCESSIVE PULSES
% freq_P = 5;                     % frequency of pulse
% period = 1/freq_P;               % pulsation period
% rest_time = 1*period;            % rest time (no pulsation)
% tot_period = period + rest_time; % total period (pulsation + rest time)
% t_mod = mod(t,tot_period);       % mod out time to get time in each pulse
% 
% if t_mod <= tot_period
%     if t_mod <= period
%         uX_Tar = -Umax * (sin(freq_P*2*pi*t)) * ( (Ly/2+w/2) - y )*( (Ly/2-w/2) - y ); % Only external forces in x-direction
%     else
