...
[xLag,yLag,Concentration] = give_Me_Immsersed_Boundary_Geometry_and_Concentration(ds,Nx,Lx,dx);
...
function C = give_Me_Initial_Concentration(Lx,Nx,dx,buff)
...
ySide = Buff:ds:Lx-Buff;
xLeft = Buff*ones(1,length(ySide));
xRight= (Lx-Buff)*ones(1,length(ySide));
...
C = give_Me_Initial_Concentration(Lx,Nx,dx,Buff);
