...
%--------------------------------------
xLagT = 0:5*ds:Lx;
yLag1 = (0.125+OffY)*ones( size( xLagT ) );
yLag2 = (0.250+OffY)*ones( size( xLagT ) );
yLag3 = (0.375+OffY)*ones( size( xLagT ) );
%
...
%     end
    ...
    if yPt < ( 0.125+Amp*sin(2*pi*xPt/lambda) + OffY ) 
        C(yInd,xInd) = 0;
    elseif yPt < ( 0.25+Amp*sin(2*pi*xPt/lambda) + OffY )
        C(yInd,xInd) = 1*Scale;
