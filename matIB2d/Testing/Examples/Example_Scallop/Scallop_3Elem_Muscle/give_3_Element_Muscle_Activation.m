...
function [Fm,on,PE_coeff,af_Val] = give_3_Element_Muscle_Activation(v,LF,LFO,SK,a,b,Fmax,current_time,xPt,xLag,i)
...
[af_Val,on,PE_coeff] = give_Traveling_Triggering_Coefficient(current_time,xLag,xPt,i);

% Actually Compute Muscle Force %
%Fm = size( xLag );
Fm = af_Val * abs( Fmax*F1*F2 );
%Fm(2) = af_Val * abs( Fmax*F1*F2 );
...
function [af_Val,on,pE_Coeff] = give_Traveling_Triggering_Coefficient(current_time,xLag,xPt,i)
...
if t < 0.04
    ...
    if i==1
        ...
        af_Val = 0;
        pE_Coeff = tanh(100*(t-0.6) );
        on = 1;
        
    elseif i==2
        
        af_Val = 0;
        pE_Coeff = tanh(60*(t-0.6) );
        on = 1;

    elseif i==3
            
        af_Val = 0;
        pE_Coeff = tanh(20*(t-0.6) );
        on = 1;
