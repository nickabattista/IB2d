...
if first == 1
    ...
    yMin = 0.1;  yMax = 0.13; xMin = 0.41; xMax = 0.59;
    ids1 = give_Me_Indices_To_Apply_Force(x,y,xMin,xMax,yMin,yMax);
    N1 = length(ids1);
    
    % Compute Where You Want to Apply Force
    yMin = 0.1;  yMax = 0.13; xMin = 0.41+0.3; xMax = 0.59+0.3;
...
%uY_Tar = -Umax * (5*tanh(t)) * ( (Lx/2+w/2+yShift) - x )*( (Lx/2-w/2+yShift) - x ); % Only external forces in y-direction

uY_Tar = -Umax * (0.5*abs(sin(20*pi*t))) * ( (Lx/2+w/2+yShift) - x )*( (Lx/2-w/2+yShift) - x ); % Only external forces in y-direction
