...
void savevtk_scalar(int arrayRow,int arrayCol,double* array, char* filename, char* colorMap,double dx, double dy){
    ...
    fprintf(fptr,"SPACING   %.15f%.15f   1.000\n",dx, dy);
    fprintf(fptr,"\n");
    ...
    for(int i = 0; i < arrayRow; i++){
        for (int j = 0; j < arrayCol; j++){
            fprintf(fptr, "%.15f ", array[i*arrayCol+j]);
        }
        fprintf(fptr,"\n");
    }
...
void savevtk_vector(int Xrow, int Xcol, int Yrow, int Ycol, double* X, double* Y,char* filename,char* vectorName,double dx,double dy){
    ...
    for (int i = 0; i< Xrow;i++){
        for (int j = 0; j<Xcol;j++){
            fprintf(fptr,"%.15f ",X[i*Xcol+j]);
            fprintf(fptr,"%.15f ",Y[i*Xcol+j]); /*The length of the number should e-15 !!!!!!!!! */
            fprintf(fptr,"1");
        }
        fprintf(fptr,"\n");
    }
...
for(int i = 0; i <100; i++){
 ...
 /*savevtk_points_connects_write(50, 50, x1, filename, vectorname, connectsMat1);*/
 /*savevtk_points_write(50,x1,filename,vectorname);*/
 /*savevtk_vector(50, 3, 50,3, x1, x1,filename,vectorname,3.0,3.0);*/
 savevtk_scalar(50,3,x1, filename, colorMap,3.0, 3.0);
 return 0;
 }
