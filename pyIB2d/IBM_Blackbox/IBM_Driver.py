...
def savevtk_points( X, filename, vectorName):
    ...
    if C_flag:
        ...
        cells = np.ravel(np.stack((np.ones(N,dtype=np.int64), np.arange(N,dtype=np.int64)),
                         axis=-1))
        # Create cell structure with args: number of cells, connectivity list
        vtk_cells.SetCells(N, numpy_support.numpy_to_vtkIdTypeArray(cells))
        vtk_obj = vtk.vtkUnstructuredGrid()
        # Set cells with cell types, cell structure
        vtk_obj.SetCells(np.ones(N, dtype=int), vtk_cells)
        vtk_obj.SetPoints(vtk_points)
        # write out
        writer = vtk.vtkGenericDataObjectWriter()
        writer.SetFileName(filename)
        writer.SetHeader(vectorName+". Generated by IB2d Python (Battista, Strickland)")
