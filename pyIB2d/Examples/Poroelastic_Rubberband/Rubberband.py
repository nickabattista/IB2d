...
def Rubberband():
    ...
    alpha = 250000              # Brinkman coefficient
...
def print_Lagrangian_Porosity(xLag,alpha,struct_name):
    
    N = len(xLag)
    
    with open(struct_name + '.poroelastic','w') as porous_fid:
        porous_fid.write('{0}\n'.format(N))
        
        #Loops over all Lagrangian Pts.
        for s in range(N):
            porous_fid.write('{0:d} {1:1.16e}\n'.format(s,alpha))
...
def give_Me_Immsersed_Boundary_Geometry(N,rmin,rmax):
    ...
    t = 2*pi/N
    xLag = 0.5 + rmax * np.cos(2*pi/N*np.arange(N))
    yLag = 0.5 + rmin * np.sin(2*pi/N*np.arange(N))
